...
$.ajaxSetup({error:
function(a,b,c){if(!c)try{c=JSON.parse(a.responseText).message;}catch(d){}if(b==='abort')return false;else if(c)lyr.alert('Error',c);else if(a.status===403)lyr.alert('No access','We鈥檙e sorry, you do not have access to this functionality.');else if(a.status===404)lyr.alert('Page not found','We鈥檙e sorry, we could not find the page. Please try again later.');else if(a.status===500)lyr.alert('Server Error','We鈥檙e sorry, we encountered a server error while processing your request. We will look into this. Please try again later.');else if(b==='parsererror')lyr.alert('Parse Error','We鈥檙e sorry, we were unable to process this request. Please try again later.');else if(b==='timeout')lyr.alert('Timeout Error','We鈥檙e sorry, the page did not load in time. Please try again later.');},crossDomain:false,beforeSend:
...
lyr.gaq.logError=
function(a,b,c){if(!$('html').hasClass('ie-nosupport'))if(a&&b)lyr.gaq.trackEvent('Exception',a+' 鈥� '+location.pathname,b+'#'+c);
...
function(){a.parent().load(a.attr('href'));a.parent().text('Resending verification email鈥�');
...
function(a){var b,c=a.height();if(c>140){a.css({height:111,overflow:'hidden'});b=$('<a href="#expand">View more鈥�</a>');b.css({position:'relative',top:-22});b.insertAfter(a);b.on('click',
...
function(){var a,b;this.contentWrap=$('<div>').addClass(this.ns+'content');this.closeButton=$();if(this.iSettings.closeButton)this.closeButton=$('<div>').addClass(this.ns+'close').text('脳');a=$('<section>').data({lightbox:this}).attr({id:this.iSettings.id}).addClass(this.ns+'lightbox').addClass(this.iSettings.addClass).append(this.closeButton,this.contentWrap).appendTo('body').css({opacity:0});
